...
public class HabitDropController {
    ...
    public String habitDrops(@AuthenticationPrincipal UserCredentialsDto user, Model model){
        List<HabitDropDto> habits = habitDropService.getAll(user.getId());
    ...
    public String viewAddForm(Model model){
        model.addAttribute("habit",new HabitDropAddDto());
        return "habit-drop-add-form";
    }

    @PostMapping("/add")
    public String addHabitDrop(@ModelAttribute HabitDropAddDto dto, @AuthenticationPrincipal UserCredentialsDto user){
    ...
    public String editHabitDrop(@PathVariable("id") Long habitId, @AuthenticationPrincipal UserCredentialsDto user, Model model){
        HabitDropEditDto habitForEdit = habitDropService.getHabitForEdit(habitId, user.getId());
        model.addAttribute("habit", habitForEdit);
        return "habit-drop-edit-form";
    }

    @PostMapping("/edit")
    public String editHabitDrop(@ModelAttribute HabitDropEditDto habit, @AuthenticationPrincipal UserCredentialsDto user){
